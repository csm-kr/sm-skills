...
- 원문이 한국어이면 의미를 유지한 자연스러운 영어 이미지 편집 프롬프트로 번역한다.
- 원문이 이미 영어여도 모호한 대상이나 원하는 속성만 최소한으로 보충한다.
- 프롬프트는 선택한 네모 crop 안에 생성할 최종 모습만 설명하며 한두 문장으로 짧게 쓴다.
- 전체 이미지, 박스 밖 배경, 자세, 조명 등의 장황한 보존 지시는 넣지 않는다. 박스 밖 보존은 stitch 단계가 담당한다.
- 예: `모자를 대머리로 변경` → `Replace the flat cap with a natural bald scalp matching the skin tone and head shape of the elderly man.`
- 사용자가 요청하지 않은 디자인이나 의미를 새로 만들지 않는다.
- 외부 레퍼런스가 있으면 어떤 형태·재질·색·스타일을 가져올지 명시한다.
