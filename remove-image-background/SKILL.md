...
사용자가 파일 경로 대신 드래그 앤 드롭을 원하면 아래 명령을 실행한 상태로 유지한다. 기본 브라우저에 로컬 입력 화면이 열리고, 설정된 입력 폴더로 이미지가 저장된 뒤 화면 서버가 종료된다.

```bash
<python> <skill-directory>/scripts/interactive_remove.py \
  --project-root <project-root>
```

stdout 마지막 줄의 입력 이미지 경로를 아래 실행 명령에 전달한다. 이 화면은 Windows/macOS/Linux에서 동일하게 동작한다.
