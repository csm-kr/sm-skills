...
기본 캔버스는 `800x2400` 세로형이다. 10장 모두 같은 굵은 한글 산세리프 계열, 동일한 헤드라인 굵기, 모서리·카드 문법, 팔레트와 사진 조명을 유지한다. 장별 내용은 달라도 한 브랜드의 연속 상세페이지처럼 보여야 한다.

## 카피와 가시성 기획

이미지를 만들기 전에 10장 각각의 정확한 한글 카피와 배치를 확정한다. 카피는 `prompt.txt`의 문제 인식 → 필요성 → 제품 소개 → 상황 비교 → 장점 → 차별점 → 추천 대상 → 활용 → 구매 결정 흐름을 따른다.
...
- 전체 10장의 헤드라인을 같은 한글 산세리프 계열과 같은 시각적 무게로 통일한다. 800px 원본 기준 `H1 72~104px`, `H2 44~64px`, `BODY 32~40px`, `라벨·캡션 최소 32px`의 토큰을 고정한다.
...
|---|---|---|---|---|
| 1 | 고객 불편 후킹 | 불편 상황, 큰 헤드라인, 공감 설명, 칩 2~3개 | H1·BODY·공감 칩 | 공포 마케팅, 질병·신체 변화 금지 |
| 2 | 상품이 필요한 이유 | 선택 기준 또는 기존 무드의 아쉬움 카드 3개 | H1·BODY·카드 3개 | 경쟁사 비방과 창작 수치 금지 |
| 3 | 상품 메인 소개 | 대표 제품 히어로, 상품명, 핵심 장점 3~4개 | 상품명·설명·검증된 포인트 | 입력 순서와 의미를 바꾸지 않음 |
| 4 | 상황·스타일 | 착장·상황 2~3개를 나누고 장면별 캡션 표시 | H1·BODY·장면 제목·캡션 | 신체 Before/After, 효과 보장 금지 |
| 5 | 구조 디테일 | 실제 디테일 3~4컷과 각각의 설명 | H1·BODY·디테일 제목·설명 | 없는 테스트 결과나 기능 금지 |
| 6 | 컬러·질감·실루엣 | 에디토리얼 히어로와 시각 포인트 3개 | H1·BODY·시각 캡션 | 근거 없는 내구·방수·항균·냉감·보온 금지 |
| 7 | 차별점 비교 | 일반적 무드와 현재 제품의 3~5개 중립 비교 | H1·BODY·비교 행 | 특정 경쟁 제품 재현·비방, 근거 없는 O/X 금지 |
| 8 | 추천 대상 | 생활·옷차림·취향 기반 카드 4~6개 | H1·BODY·추천 카드 | 사회적 증거, 임의 리뷰·판매량 금지 |
| 9 | 착용법·활용 | 단계 또는 실제 사용 장면 3~5개 | H1·BODY·단계·캡션·주의 | 무관한 장면과 과도한 연출 금지 |
| 10 | 구매 결정 지원 | 제품 히어로, 장점 3~4개 요약, 중립 CTA | H1·BODY·요약 카드·CTA | 가격·할인·배송·이벤트·보증 창작 금지 |

각 장은 `800x2400` 독립 이미지여야 하며, 10장을 이어 보면 문제 인식에서 구매 결정까지 자연스럽게 흐르게 한다. 파일명에는 장 번호를 사용하되 이미지 픽셀 안에는 `Page`, 장 번호 또는 페이지 번호를 렌더링하지 않는다.
...
- WEB_CONFIRMED_PRODUCT: optional M1 or M2 evidence only; supporting role and never higher priority than ORIGINAL_PRODUCT.
...
- use one consistent modern Korean sans-serif family shared by all ten images
...
10장이 생성되면 에이전트가 결과를 직접 열어 장별로 확대 검수한다. 파일 존재나 스크립트 통과만으로 시각 QA를 대체하지 않는다.

1. 파일이 열리고 실제 래스터 크기가 `800x2400`인지 확인한다.
2. 승인된 한글 카피를 글자 단위로 대조한다. 깨진 한글, 누락, 중복, 오탈자, 의사문자와 추가 텍스트가 없어야 한다.
3. 10장의 헤드라인·본문·카드·캡션이 같은 한글 산세리프 계열, 정해진 굵기·크기 토큰과 정렬 원칙을 유지하는지 확인한다.
