...
# Heuristic regex
# 경로 참조: `src/app.py`, `./docs/x.md`, `.claude/commands/harness.md` 등.
# - 앞의 lookbehind 에 `.` 를 포함해 단어 중간에서 시작하지 않도록 하고, 첫 세그먼트에
#   optional `\.?` 를 두어 `.claude/` 같은 dot-dir 의 점을 보존한다.
# - 확장자 뒤 `(?![A-Za-z0-9])` 경계로 `index.json` 이 `.js` 로 절단되는 것을 막는다.
RE_PATH_REF = re.compile(
    r"(?<![A-Za-z0-9_/.])"
    r"((?:\./|\.?[A-Za-z0-9_]+/)[A-Za-z0-9_./-]+\.(?:tsx|jsx|json|py|ts|js|md|sql|yaml|yml|toml|html|css|sh|go|rs|java|kt|rb|php))"
    r"(?![A-Za-z0-9])"
)
# fenced code block (```…```): 명령어 예시·디렉토리 트리가 들어있어 실제 참조 링크가 아니다.
# E1 참조 검증 전에 제거해 예시 경로를 hallucination 으로 오인하지 않는다.
RE_FENCE = re.compile(r"```.*?```", re.DOTALL)
# tribal knowledge store 파일: ADR / DECISIONS / RULES / STATE / LEARNINGS / LOG / MEMORY.
RE_TRIBAL_FILE = re.compile(
    r"(?:^|/)(?:ADR|DECISIONS?|RULES|STATE|LEARNINGS?|LOG|MEMORY)\.md$", re.IGNORECASE
)
...
def _keep_dir(name: str) -> bool:
    # '.' (dotdir) 와 '_' (아티팩트/스크래치: _handoff_unzip, _shots, __tests__ 등) 제외.
    return name not in IGNORE_DIRS and not name.startswith(".") and not name.startswith("_")
...
def find_core_modules(repo: Path) -> list[Module]:
    ...
    for d in candidates:
        ...
        for r, dirs, files in os.walk(d):
            dirs[:] = [x for x in dirs if _keep_dir(x)]
            for f in files:
...
def score_c(modules: list[Module], repo: Path) -> CategoryScore:
    ...
    # Detect MEMORY.md / ADR / decisions — 디렉토리형(docs/adr/) 뿐 아니라 파일형
    # (docs/agent/ADR.md, docs/RULES.md, docs/LOG.md …) 도 tribal store 로 인정한다.
    claude_mem_dir_hits = list(repo.glob(".claude/memory*"))
    tribal_files: list[Path] = []
    for base in (repo / "docs", repo / ".claude"):
        if base.exists():
            for p in base.rglob("*.md"):
                if any(seg in IGNORE_DIRS for seg in p.parts):
                    continue
                if RE_TRIBAL_FILE.search(p.as_posix()):
                    tribal_files.append(p)
    has_memory = (repo / "MEMORY.md").exists() or any(
        p.name.upper() == "MEMORY.MD" for p in tribal_files
    )
    adr_dirs = [
        repo / "docs" / "adr",
        repo / "docs" / "decisions",
    ...
    has_adr = any(d.exists() for d in adr_dirs) or bool(tribal_files)
...
def score_d(repo: Path, context_files: list[Path]) -> CategoryScore:
    # docs/ 하위 어디에 있든 아키텍처/의존성/데이터플로우 문서를 인정한다
    # (예: docs/dev/ARCHITECTURE.md). 루트/직속만 보던 기존 로직의 사각지대.
    docs = repo / "docs"
    docs_md = list(docs.rglob("*.md")) if docs.exists() else []

    def _docs_has(*names: str) -> bool:
        wanted = {n.lower() for n in names}
        return any(p.name.lower() in wanted for p in docs_md)
    ...
    has_arch = any((repo / p).exists() for p in (
        ...
        "ARCHITECTURE.md", "dependency-graph.md", "data-flow.md", "data_flow.md",
    )
    # mermaid 는 context + docs 전체에서 탐지. mermaid 가 없어도 시퀀스/데이터플로우
    # 문서(ASCII 다이어그램 포함)는 시각적 의존도 표현으로 동등 가점한다.
    has_mermaid = any(RE_MERMAID.search(read_text(p)) for p in list(context_files) + docs_md)
    has_flow_doc = _docs_has(
        "SEQUENCE_DIAGRAM.md", "sequence-diagram.md", "sequence.md",
    ...
    if has_arch:
        pts += 6
    if has_mermaid or has_flow_doc:
        pts += 3
    ...
    if not has_mermaid and not has_flow_doc:
        findings.append("mermaid / 시퀀스 다이어그램 없음 — 시각적 의존도 표현 부재")
...
def score_e(repo: Path, context_files: list[Path]) -> CategoryScore:
    ...
    for p in context_files:
        # fenced code block(명령어 예시·디렉토리 트리)은 실제 참조가 아니므로 제거 후 검증.
        text = RE_FENCE.sub(" ", read_text(p))
